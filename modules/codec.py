...
class EncFFMPEG(Enc):
    def __init__(self, path, output, width=-1, height=-1, out_fps=-1, start_time=0, duration=10):
        ...
        audio_exists = len([s for s in self.probe['streams'] if s['codec_type'] == 'audio'])
        if audio_exists:
            ...
            self.aud_rate = None
            self.aud_channels = None
            self.aud_codec_name = None
            self.aud_bitrate = None
            self.aud_channel_layout = None
            self.aud_sample_format = None
